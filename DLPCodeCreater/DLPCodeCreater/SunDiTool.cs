    ...
    public partial class Form1 : Form
    {
        ...
        private void DownloadAndUpdate()
        {
            ...
            try
            {
                ...
                string fileId = "1-dhzcE3XKboYtotxQ4ZAi8XgJ1MH0opl";  // Google Drive fileID
        ...
        private void btn_install_component_Click(object sender, EventArgs e)
        {
            try
            {
                string cmd = "start cmd /k \"cd /d D:\\DLP_Git\\dlp-develop\\DLP.Web\\DLP.Web.AppPortal\\ClientApp && npm i -f \"";
                ExecuteCommand(cmd);
            }
            catch (Exception)
            {
        ...
        #region back end tool


        //解密資料
        public string DecryptStringAES(string cipherText)
        {
            string jkeybytes = jsonData.Keybytes;
            //dlplat77wjeojd89
            byte[] keybytes = Encoding.UTF8.GetBytes("dlplat77wjeojd89");   //自行設定，但要與JavaScript端 一致
            byte[] iv = Encoding.UTF8.GetBytes("dlplat77wjeojd89"); // 自行設定，但要與JavaScript端 一致
            byte[] encrypted = Convert.FromBase64String(cipherText);
            string decriptedFromJavascript = DecryptStringFromBytes(encrypted, keybytes, iv);

            string returnValue = string.Format(decriptedFromJavascript);
            if (returnValue.IndexOf("%7B") > -1 || returnValue.IndexOf("%7D") > -1 || returnValue.IndexOf("%7E") > -1 || returnValue.IndexOf("%7F") > -1)
            {

                returnValue = returnValue.Replace("%7B", "{").Replace("%7D", "}").Replace("%7E", "\\\\").Replace("%7F", "\\\"");
            }
            return returnValue;
        ...
        private void btn_decryptstringAES_Click(object sender, EventArgs e)
        {
            try
            {
                if (this.tbx_tab8_encryption.Text != "")
                {
                    this.tbx_tab8_decryption.Text = DecryptStringAES(this.tbx_tab8_encryption.Text);
                }
                else
                {
                    MessageBox.Show("請先輸入加密字串!");
                }
            }
            catch (Exception)
            {
                MessageBox.Show("字串有誤!");
            }
